...
# استيراد القاموس الذهبي الموثق من ملفك
try: 
...
# إيموجي الأرقام لضمان التنسيق الجمالي برمجياً
...
def run_saudi_analyzer():
    try:
        ...
        # 2. تجهيز البيانات (نرسل الرموز فقط لمنع AI من التخمين)
        ...
        for line in lines:
            ...
            if match:
                ...
                if symbol in tadawul_map:
                    # نرسل الرمز فقط؛ الذكاء الاصطناعي لا يرى الاسم أبداً
                    ai_input += f"ID:{symbol} | Data:{line}\n"

        if not ai_input: return

        # 3. برومبت "المحلل التقني الصرف"
        prompt = f"""
        Analyze these symbols technically. Return only the top 10 positive ones.
        Output format: SYMBOL|TARGET|STOP|ANALYSIS
        Strict Rule: NO NAMES, NO INTROS.
        Data:
        {ai_input}
        """
        ...
        # 4. بناء الرسالة (بايثون هو من يطبع الأسماء من ملفك الموثق)
        ...
        for row in raw_output.strip().split('\n'):
            ...
            if len(parts) >= 4 and count < 10:
                ...
                # مطابقة الرمز بالاسم من ملفك (هنا يستحيل الخطأ)
