...
def get_stock_details(raw_line):
    ...
    if match:
        ...
        if details:
            # نرسل الاسم المعتمد في ملف الإكسل فقط
...
def run_saudi_analyzer():
    try:
        ...
        for line in lines:
            if len(line) > 5:
                ...
                if symbol:
                    ...
                    # نرسل "الاسم العربي" كمعطى ثابت لا يقبل التأويل
                    market_sections[market].append(f"الاسم المعتمد: {name} | الرمز: {symbol} | البيانات الفنية: {line}")
        ...
        # 4. البرومبت الصارم (نسخة البروفيسور المحدثة)
        prompt = f"""
        أنت محلل فني آلي "قناص السوق السعودي". 
        المصدر الوحيد للحقائق والمسميات هو هذه القائمة:
        {full_context}

        التعليمات الصارمة لمنع الهلوسة:
        1. التزم بكلمة "الاسم المعتمد" المذكورة بجانب كل رمز حرفياً. 
        2. يمنع منعاً باتاً استبدال الأسماء المرفقة بأي أسماء من ذاكرتك (مثلاً: لا تستخدم بدجت بدل المتحدة، ولا المعمر بدل توبي).
        3. قم بترقيم الأسهم في كل قسم باستخدام أرقام الإيموجي (1️⃣، 2️⃣، 3️⃣، 4️⃣، 5️⃣...).
        4. اعرض فقط الشركات ذات المؤشرات الإيجابية الفنية.
        5. ممنوع ذكر أي شركة لم تظهر في البيانات المرفقة أعلاه.
        ...
        *تقرير الفرص اللحظية*
        ...
        ### [اسم القسم/السوق]
        [رقم الإيموجي] • [الاسم المعتمد من البيانات] ([الرمز]) | [السعر] ريال
