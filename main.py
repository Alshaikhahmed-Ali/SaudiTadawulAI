...
def scrape_argaam_news(symbol):
    news = []
    try:
        url = "https://www.argaam.com/ar/company/news/" + symbol
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
        print("    - جلب الأخبار من أرقام...")
        ...
        if response.status_code == 200:
            ...
            if news:
                print("    ✓ تم جلب " + str(len(news)) + " خبر من أرقام")
    except Exception as e:
        print("    [WARNING] فشل جلب الأخبار من أرقام: " + str(e))
    return news

def scrape_tadawul_events(symbol):
    events = []
    try:
        url = "https://www.saudiexchange.sa/wps/portal/saudiexchange/listing/company-profile-main/" + symbol
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}
        print("    - جلب الأحداث من تداول...")
        response = requests.get(url, headers=headers, timeout=10)
        if response.status_code == 200:
            soup = BeautifulSoup(response.content, 'html.parser')
        ...
        print("    [WARNING] فشل جلب الأحداث من تداول: " + str(e))
    return events
...
def get_company_info_from_gemini(symbol, info, news_context=""):
    try:
        ...
        print("    [WARNING] فشل تحليل Gemini: " + str(e))
    return []
...
def send_to_telegram(symbol, info, price, target, stop, analysis, index, company_data):
    ...
    if success:
        print("[SUCCESS] ✓ تم إرسال " + symbol + " بنجاح (MarkdownV2)")
        return True
    print("[WARNING] فشل MarkdownV2، محاولة التنسيق البسيط...")
    success, response = send_telegram_message(message_simple, None)
    if success:
        print("[SUCCESS] ✓ تم إرسال " + symbol + " بنجاح (تنسيق بسيط)")
        return True
    else:
        print("[ERROR] فشل الإرسال تماماً: " + str(response))
        return False
...
def run_saudi_analyzer():
    ...
    try:
        ...
        print("  ✓ عدد الأسطر: " + str(len(lines)))
        ...
        if g_res.status_code == 200:
            try:
                ...
                print("  ✓ تم استخراج " + str(len(final_results)) + " نتيجة")
            except:
                pass
            ...
            for s in top_list[:3]:
                try:
                    ...
                    if p > 0:
                        ...
                        stop = round(p * 0.97, 2)
                        final_results.append(s + "|" + str(target) + "|" + str(stop) + "|قيد المراقبة الفنية")
                except:
                    continue
        ...
        print("\n" + "=" * 60)
        print("✅ انتهى التحليل!")
        print("📊 تم إرسال " + str(sent_count) + " توصيات")
        print("=" * 60)
    except Exception as e:
        print("\n[ERROR] ❌ خطأ: " + str(e))
        import traceback
